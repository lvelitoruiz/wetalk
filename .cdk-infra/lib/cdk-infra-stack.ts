...
export class CdkInfraStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props: CloudStackProps) {
    ...
    const cfredirects_rewrites = new cloudfront.Function(
      this,
      'cfredirects_rewrites',
      {
        functionName: `${props.shortRegion}-cfredirects_rewrites-${props.appName}-${props.stage}`,
        code: cloudfront.FunctionCode.fromFile({
          filePath: './cfFunctions/redirects_rewrites.js',
        }),
      }
    );
    const cfsecurity_headers = new cloudfront.Function(
      this,
      'cfsecurity_headers',
      {
        functionName: `${props.shortRegion}-cfFunction_viewerResponse-${props.appName}-${props.stage}`,
        code: cloudfront.FunctionCode.fromFile({
          filePath: './cfFunctions/security_headers.js',
        }),
      }
    );
