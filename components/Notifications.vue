...
<template>
  <div
    class="fixed top-[68px] lg:absolute lg:left-[-40px] lg:top-[70px] z-50 left-[0]"
  >
    <div class="hidden lg:block lg:ml-[40px] ml-[180px] triangulo-up z-10" />
    <div
    ...
    >
      ...
      <div class="flex items-center gap-3 px-4 py-2">
        ...
        >
          <div
            class="rounded-full box-border h-7 flex items-center gap-1 cursor-pointer"
            :class="[
              {
                'border border-[#E50A17] bg-[#E50A17] px-4 py-1 text-white':
                  notification.active,
          ...
          >
            ...
            >
              <span
                class="font-medium text-[12px] font-zizou-regular"
                :class="[
                  {
      ...
      <div class="black-scroll overflow-y-auto max-h-[350px]">
        <div v-if="finalItems !== null">
          ...
          <div
            v-for="item in finalItems[1]"
            :key="item"
          >
            <div
              @click="
                eventClickCardNotification(
                  item.mensaje,
                  item.tipo,
                  item.titulo,
                  item.url,
                  item.id,
                  item.codigo_campania,
                  item.registros_totales,
                  item.poblacion_fija
                )
              "
              class="py-2 flex items-center px-4 cursor-pointer gap-5 border-b border-white"
              :class="{ 'bg-[#D7E9FB]': item.status }"
            >
              <span
                class="min-w-[12px] min-h-[12px] bg-[#E50A17] rounded-full"
              />
              <div>
                ...
                <div class="flex items-center gap-1">
                  ...
                  >
                    ...
                    />
                    <span
                      class="text-xs font-publicSans font-medium leaning-none"
                    >{{ item.tipo }}</span>
                  </div>
            ...
            >
              ...
              <div>
                ...
                <div class="flex items-center gap-1">
                  ...
                  >
                    <i class="text-[#554A00] text-sm icon-book-open" />
