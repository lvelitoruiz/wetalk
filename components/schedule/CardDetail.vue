...
<template>
    <div v-if="isRepeated(course.IdSesion, course.NombresImag, course.ApePatImag)"
        @click="showPopup"
        :role="smallerThanLg ? 'div' : 'button'"
        :tabindex="smallerThanLg ? 1 : ''"
        class="card-sc-course-detail"
        :class="{
            'lg:h-[58px] lg:flex lg:items-center': courseDuration(course.HoraInicioSesion, course.HoraTerminoSesion) === 1,
            'lg:h-[118px]': courseDuration(course.HoraInicioSesion, course.HoraTerminoSesion) === 2,
            'lg:h-[178px]': courseDuration(course.HoraInicioSesion, course.HoraTerminoSesion) === 3,
            'lg:h-[238px]': courseDuration(course.HoraInicioSesion, course.HoraTerminoSesion) === 4,
        }"
        :style="{
            top: getTopStyle(course.HoraInicioSesion),
        }"
    >
        <h3 class="title-curso" :title="course.DescEspecialCurso">{{ course.DescEspecialCurso }}</h3>
        <div
            :class="{
                'one-hour-course': courseDuration(course.HoraInicioSesion, course.HoraTerminoSesion) === 1 && !smallerThanLg,
        ...
        >
            ...
            <p>
                ...
                {{ getHourMinutes(course.HoraInicioSesion) }} - {{ getHourMinutes(course.HoraTerminoSesion) }}
            </p>
            <p><span class="text-secondary-400">Aula:</span> {{ course.CodAula }}</p>
            <p><span class="text-secondary-400">Campus:</span> {{ sedes[course.CodLocal] }}</p>
            <p>
                <span class="text-secondary-400">Profesor: </span>
                <span v-if="Boolean(listP[course.IdSesion].docentes.length)">
                    <template v-for="(docente, k) in listP[course.IdSesion].docentes" :key="k">
                        {{ docente }}{{ k === listP[course.IdSesion].docentes.length - 1 ? '' : ','}}&nbsp;
                    </template>
                </span>
                <span v-else>-</span>
            </p>
        </div>
        <div class="hidden lg:block absolute text-primary mt-auto ml-auto mr-[2px] lg:mr-1 bottom-2 right-1 rounded-[4px] hover:bg-[#E50A17] hover:text-[#FFFFFF]" v-bind:class="{ 'bg-[#E50A17] !text-[#FFFFFF]': popupIsVisible }">
            <NuxtIcon class="text-[20px]" name="defaultDia"  />
        </div>
        <SchedulePopUp
