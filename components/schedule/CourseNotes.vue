...
const showCourse = async (curso) => {
    ...
    if(showItem.value) {
        ...
        } else if(Boolean(matricula1.value?.error)) {
            errorMsg.value = {...matricula1.value.error, flag: matricula1.value.flag}
        }
    }
};
...
<template>
  <div class="cursor-pointer" @click.prevent="showCourse(cursosItems)">
    <div class="flex justify-between">
      <h3
        class="font-solano text-xl uppercase leading-[1]"
        v-bind:class="{ 'text-primary': showItem }"
      >
  ...
  <div class="content_detalls text-sm pt-4 mt-2 pb-4 border-t-neutral-2 border-t-[1px]" v-if="showItem">
    <div class="content_note">
        <div v-if="loading" class="min-h-[80px] flex justify-center items-center">
            <img class="w-[50px]" src="~/assets/images/loading.gif" alt="Cargando">
        </div>
        <div class="flex" v-else-if="Boolean(errorDate)">
            <NuxtIcon class="text-[35px] mr-2" filled name="IconNoService" />
            <div>
                <p class="font-bold">Lo sentimos, no pudimos cargar tus notas</p>
                <p>Inténtalo de nuevo en unos minutos</p>
            </div>
        </div>
        <div v-else-if="Boolean(errorMsg)" class="flex">
            <NuxtIcon v-if="!errorMsg.flag" class="text-[27px] mr-2" filled name="IconNoService" />
            <NuxtIcon v-else class="text-[27px] mr-2" filled name="IconClock2" />
        ...
        <div v-else>
            <div class="c_notas grid gap-4">
                ...
                >
                    ...
                    <span class=""
                        :class="{
                        'text-secondary': Number(item.nota) > 12.5 || isNaN(Number(item.nota)),
                        'text-primary': Number(item.nota) <= 12.5 && Number(item.nota) > 0,
            ...
            <div class="flex c_footer_nota mt-5 justify-between items-center">
                <div class="flex c_faltas items-center">
                    <div class="">
                        <span class="px-1 lg:px-[2px] text-[12px] xl:text-[15px] leading-[1]">Inasistencias</span>
                    </div>
                ...
                <div class="flex c_promedio items-center">
                    <div class="">
                        <span class="px-1 lg:px-[2px] text-[12px] xl:text-[15px] leading-[1]">Promedio final</span>
                    </div>
                    <span class="number text-center xl:ml-1 text-white" :class="{
                        'bg-secondary': Number(detailsNote.notaFinal) > 12.5 || isNaN(Number(detailsNote.notaFinal)),
