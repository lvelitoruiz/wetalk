...
const hours = ref({
    ...
    "19": 55, "20": 55, "21": 55, "22": 55, "23": 55, "24": 0
})
...
const sessionDate = computed(() => {
    if (states.coursesListDay?.empty) {
        ...
        return new Date(today.setDate(today.getDate() - today.getDay() + states.day + 1)).toDateString();
    }
...
<template>
    <div>
        <div class="bg-[#5598FF] box-border w-full grid grid-cols-2 gap-x-3 gap-y-4 xl:flex xl:justify-between px-3 xl:px-10 py-2 mb-0">
            <ScheduleDayWeekTab :changeView="changeView" :active="active" />
            <!-- Filtro  -->
            <ScheduleCourseFilter v-if="active === 'day'" :selectedCourse="selectedCourseDay" :handleCourse="handleFilter"
                :allCourses="states.dayFilter" />
            <ScheduleCourseFilter v-else :selectedCourseWeek="selectedCourseWeek" :handleCourse="handleFilterWeek"
                :allCourses="allCourses" />
            <!-- Tab Dia/Mes -->
            <ScheduleDayControls :handleChange="handleChange" :day="states.day"
                :sessionDate="active === 'day' ? sessionDate : new Date().toDateString()" :courses="states.fullWeek"
                :active="active === 'week'" />
        </div>
        <!-- Calendario dia - semana -->
        <ScheduleDayBoard v-if="active === 'day'" :filter="selectedCourseDay" />
        <ScheduleWeekBoard v-else-if="active === 'week'" :data="data" :allCourses="allCourses"
            selectedCourse="selectedCourseWeek" :filter="selectedCourseDay" :hours="hours" :handleChange="handleChange" />
    </div>
</template>