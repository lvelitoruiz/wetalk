...
if (
  Object.keys(props.allCourses).length === 0 ||
  earlierCourse.value === Infinity
) {
  ...
  <div class="flex flex-col px-2 lg:py-5 bg-white">
    ...
    >
      ...
      >
        <p
          v-if="smallerThanLg"
          class="dateList-item-text lg:mr-1"
        >
          {{ getDayName(day, true).slice(0, 1) }}
        </p>
        <p
          v-else
          class="dateList-item-text lg:mr-1"
    ...
    <div class="calendar-container">
      <!-- horas -->
      <div
        class="hours-container relative"
        ref="listHours"
      >
        <div
          class="h-[10px] border-r-[1px] border-b-[1px] border-neutral-400"
        />
        <div
          class="hour-mark"
          :style="{
        ...
        <div
          ...
          :class="{
            'h-[62px] lg:h-[55px] border-b-[1px]':
              i !== Object.keys(hours).length - 1,
          }"
        >
          <div
            class="leading-[1] text-xs lg:text-sm absolute top-[-10px] lg:top-[-10px] bg-white pr-1 w-[40px] lg:w-full"
          >
            {{ hour }}:00
          </div>
        </div>
      ...
      <div
        class="calendar-grid scroll-horario-sch relative"
        ref="coursesCalendar"
      >
        <div
          v-for="(item, ix) in store.fullWeek"
          :key="item"
          class="relative"
        >
          <div class="lg:absolute lg:top-0 w-full h-full">
            ...
            >
              <div
                class="grid-box h-[62px] lg:h-[55px] relative"
                v-if="i !== Object.keys(hours).length - 1"
              >
                <div
                  v-if="Number(hour) === getHour(nowMark)"
                  ...
                  :style="{
                    top: `${(heightBox / 60) * getMinutes(nowMark) - 1}px`,
                  }"
                />
              </div>
            </template>
          ...
          <template v-if="!item?.empty">
            <template
              v-for="(course, i) in item.ListaDTOHorarioAlumnoDet"
              :key="i"
            >
              {{ console.log(course) }}
