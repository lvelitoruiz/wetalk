...
<template>
    <div class="col-span-2 flex items-center justify-center xl:order-1">
        <button aria-label="Icon left" class="xl:px-1 pr-1" @click="handleChange(-1)" :disabled="day <= 0 || active"
        ...
        <button aria-label="Icon right" class="xl:px-1 pl-1" @click="handleChange(1)" :disabled="day >= courses.length - 1 || active"
            :class="day >= courses.length - 1 || active ? 'text-transparent' : 'text-neutral-1'"
        >
            <nuxt-icon class="text-[14px] no-fill" name="right" />
        </button>
    </div>
</template>