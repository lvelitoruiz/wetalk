...
<template>
    <div>
        ...
        >
            ...
            <div class="line" :style="{ width: `${width + 10}px` }"></div>
        </div>
        <div class="absolute w-full hidden lg:block" ref="el">
            <div v-for="(x, i) in new Array(17)" :key="i"
                class="w-full h-[60px] border-t-[1px]
                last:border-b-[1px] border-neutral-400 limit">
            </div>
        </div>
        <ScheduleCardDetail v-for="(course, i) in data.coursesSelectedDay(filter)" :key="i" :course="course" />
    </div>
</template>
