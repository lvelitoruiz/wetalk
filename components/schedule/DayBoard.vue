    ...
    >
      <p class="font-solano text-xl">
        <span
          ...
          v-if="data.day === isToday(new Date())"
        >
          HOY,
        </span>
      ...
      <p class="p-1 font-sans text-xs rounded bg-[#FFF4AA] mt-2 inline-block">
        Tienes {{ coursesToday }}
        {{ coursesToday.length >= 2 ? 'clases hoy' : 'clase hoy' }}.
      </p>
    </div>
    <div
      ref="coursesCalendar"
      class="flex pr-3 lg:pr-4"
      :class="{ 'scroll-horario-sch': !data.coursesListDay?.empty }"
    >
      <div
        v-if="!data.coursesListDay?.empty"
      ...
      >
        ...
        >
          <div
            class="leading-[1] lg:pr-3 lg:pt-[5px] text-[14px]"
      ...
      <div class="relative w-full mt-[12px]">
        <template v-if="!data.coursesListDay?.empty">
          <ScheduleDayBoardGrid
            :courses-calendar="coursesCalendar"
            :filter="filter"
          />
        </template>
        <div
        ...
        >
          <NuxtIcon
            filled
            class="text-[52px] mb-[10px] bg-[#FFFFFF] rounded-full px-[20px] pt-[20px] pb-[13px]"
            name="noData"
          />
          <p
            v-if="data.day === isToday(new Date())"
            class="font-bold text-center leading-[1.2] mb-1"
          >
            No tienes cursos asignados para hoy
          </p>
          <p
            v-if="data.day !== isToday(new Date())"
            class="font-bold text-center leading-[1.2] mb-1"
          >
            No tienes cursos asignados este día
          </p>
        </div>
      </div>
    </div>
